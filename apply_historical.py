...
def close_at(start, stock_data : pd.DataFrame, close_situation : str, close_at_end_day : bool):
    ...
    if not movement.empty:
        return pd.Series({'close_time' : movement.index[0], 'pct_change' : ((movement.iloc[0] - price.iloc[0]) / price.iloc[0])})
    
    #returns sale if closed and close is true
    return  pd.Series({'close_time' : price.index[-1], 'pct_change' : ((price.iloc[-1] - price.iloc[0]) / price.iloc[0])})
